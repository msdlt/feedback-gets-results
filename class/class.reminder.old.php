	...
	function buildReminders($reminderdata, $type = null){
		...
		if (! isset($reminderdata)){
			...
			$html = '<table><tr>
				...
				<th><div class="th">Message</div></th>';
				
			if ($type == "list"){
				$html .= '<th><div class="th">Options</div></th>';
			}	
			...
			foreach($reminderdata as $reminder){
				...
				if (isset($type)){
					...
					$html .= '<td><table><tr><td class="nostyle"><form action="./?res&add&rem='.$i.'" method="post" name="editrem">
						<input class="noborder" type="image" name="edit" src="./images/edit.png" /><input name="compulsory" type="hidden" value="1" />
<input name="addres" type="hidden" value="3" />
						</form></td>
					    <td class="nostyle"><form action="./?res&add&rem='.$i.'&del" method="post" name="delrem">
						<input class="noborder" type="image" name="del" src="./images/del.png" /><input name="compulsory" type="hidden" value="1" />
<input name="addres" type="hidden" value="3" />
						</form></td></tr></table>';
	...
	<th><div class="th">Date</div></th>
	<th><div class="th">Message</div></th>
	<th><div class="th">Options</div></th></tr>';
	...
	while ($data = mysql_fetch_array($sql)){
		...
		<td>'.date("jS M Y", strtotime($data['rem_date'])).'</td>
		<td>'.$msg.'</td>
		<td><table><tr><td class="nostyle"><form action="./?rem='.$data['rem_id'].'" method="post" name="editrem">
						<input class="noborder" type="image" name="edit" src="./images/edit.png" />
						</form></td>
					    <td class="nostyle"><form action="./?rem='.$data['rem_id'].'&del" method="post" name="delrem">
						<input class="noborder" type="image" name="del" src="./images/del.png" />
	...
	while ($data = mysql_fetch_array($sql)){
		...
		$html .= '<table><tr><th><div class="th">Resultset</div></th>
			...
			while ($data2 = mysql_fetch_array($sql2)){
				...
				$html .= '<tr><td>'.date("jS M Y", strtotime($data2['rem_date'])).'</td>
		<td>'.$msg.'</td>
		<td><table><tr><td class="nostyle"><form action="./?res&add" method="post" name="editrem">
						<input class="noborder" type="image" name="edit" src="./images/edit.png" />
						</form></td>
					    <td class="nostyle"><form action="./?res&add" method="post" name="delrem">
						<input class="noborder" type="image" name="del" src="./images/del.png" />
						</form></td></tr></table></td></tr>';
			}
		$html .= '</table>';			
...
return $html;	
	...
	if ($virtual == null){
		...
		$res = '<input type="hidden" name="addres" value="3"><input name="compulsory" type="hidden" value="1" /></form><form id="form2" name="editRem2" method="post" action="./add&res">
		<input type="hidden" name="addres" value="3"><input type="submit" name="Submit" value="Cancel">';
	}
